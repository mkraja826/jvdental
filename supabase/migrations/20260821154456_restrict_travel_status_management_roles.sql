...
begin
  ...
  if tg_op = 'UPDATE' then
    ...
    if old.status is distinct from new.status then
      ...
      if new.status in ('confirmed','completed','cancelled') and coalesce(auth.role(), '') <> 'service_role' then
        select sp.role into v_actor_role
        from public.staff_profiles sp
        where sp.user_id = auth.uid() and sp.is_active = true;

        if v_actor_role is null or v_actor_role not in ('owner','admin','coordinator','receptionist') then
          raise exception 'Travel status may only be managed by operational staff';
        end if;
      end if;
    end if;
  end if;
