...
export async function POST(request: Request) {
  try {
    ...
    if (paymentError || !payment) {
      return NextResponse.json({ error: "Payment record was not found." }, { status: 404 });
    }

    await supabase.from("booking_payments").update({ provider_payment_id: paymentId, status: "paid" }).eq("id", payment.id);
    await supabase.from("appointment_requests").update({ status: "paid" }).eq("id", requestId);
