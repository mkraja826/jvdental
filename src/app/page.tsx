...
const dionaviWorkflow = [
  { title: "CBCT + intra-oral scan", href: "/guided-implants" },
  { title: "Digital crown & implant planning", href: "/guided-implants" },
  { title: "Patient-specific surgical guide", href: "/guided-implants" },
  { title: "Guided implant placement", href: "/guided-implants" },
  { title: "Restoration-led full-arch planning", href: "/dental-implants" },
];
...
export default async function Home() {
  ...
  return <main className="home-page"><SiteHeader />
    ...
    <section className="dark-band" id="treatments"><div className="section"><p className="section-kicker">DIOnavi guided implant dentistry</p><h2 className="section-title">Digital planning that connects the scan, the final tooth and guided implant placement.</h2><p className="section-intro">JV Dental uses the DIOnavi digital implant workflow for selected cases. Following DIO’s official workflow, CBCT and intra-oral scan information can be combined for restorative-driven implant planning, followed by patient-specific surgical-guide design and guided placement where clinically appropriate.</p><div className="treatments">{dionaviWorkflow.map((t, i) => <Link className="treatment-row" href={t.href} key={t.title}><span>{String(i + 1).padStart(2,"0")}</span><strong>{t.title}</strong><b aria-hidden="true">↗</b></Link>)}</div><div className="hero__actions"><Link className="button button--light" href="/guided-implants">Explore DIOnavi at JV Dental <span aria-hidden="true">→</span></Link><a className="button button--ghost" href="https://hq.dionavi.com/" target="_blank" rel="noreferrer">Official DIOnavi website <span aria-hidden="true">↗</span></a></div></div></section>
